...
>
	...
	>
		...
		{#if badgeCount > 0}
			<span
				class="absolute -top-1 -right-1 min-w-[16px] h-4 px-1 rounded-full bg-severity-warning text-[#3a2a00] text-[10px] font-semibold flex items-center justify-center"
			>{badgeCount}</span>
		{/if}
	</button>
