<script lang="ts">
	...
	let {
		...
		parameters,
		siteParameters,
		selectedParameterId = $bindable(),
		selectedSiteIds = $bindable(),
		selectedYears = $bindable(),
		frequency = $bindable(),
		lastParamForSites = $bindable(),
	}: {
		sites: Site[];
		...
		frequency: Frequency;
		lastParamForSites: string;
	} = $props();

	// Default to the first measured parameter when nothing (or a non-measured parameter
	// carried over from another tab) is selected.
	onMount(() => {
		if (!parameters.some((p) => p.id === selectedParameterId)) {
