...
import { clickTarget, isAppClient } from '$lib/notificationClick';
...
self.addEventListener('notificationclick', (event) => {
	...
	const url = clickTarget(event.notification.data?.url, base);
	event.waitUntil(
		self.clients.matchAll({ type: 'window', includeUncontrolled: true }).then((clients) => {
			for (const client of clients) {
				if (isAppClient(client.url, base) && 'focus' in client) {
					client.focus();
					client.navigate(url);
					return;
				}
