				...
				{:else}
					{#each items as item}
						...
						<tr class="border-b border-brand-divider last:border-b-0 hover:bg-brand-bg/50">
							<td class="px-4 py-2"><a href="{base}/derived/{item.id}" class="text-brand-primary font-semibold no-underline hover:underline">{item.name}{item.units ? ` (${item.units})` : ''}</a></td>
