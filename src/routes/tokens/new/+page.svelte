<script lang="ts">
	...
	async function handleSubmit(e: SubmitEvent) {
		...
		try {
			...
			const result = await POST<{ id: string; token: string }>('/api/admin/api_tokens', payload);
	...
	{#snippet children()}
		<div class="space-y-3">
			...
			<div class="text-xs text-brand-muted">
				Example: <code class="bg-brand-bg px-1 rounded">curl -H "Authorization: Bearer {createdToken.slice(0, 8)}..." /api/service/sites</code>
			</div>
		</div>
	{/snippet}
	{#snippet actions()}
